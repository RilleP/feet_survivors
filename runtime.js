...
class WebGLInterface {
	...
	getWebGL1Interface() {
		return {
			...
			Uniform1iv: (location, values_ptr, values_len)             => { 
				var values = this.mem.loadI32Array(values_ptr, values_len);
				this.ctx.uniform1iv(this.uniforms[location], values);
            },

            Uniform1fv: (location, values_ptr, values_len)             => { 
				var values = this.mem.loadF32Array(values_ptr, values_len*1);
				this.ctx.uniform1fv(this.uniforms[location], values);
            },
            Uniform2fv: (location, values_ptr, values_len)             => { 
				var values = this.mem.loadF32Array(values_ptr, values_len*2);
				this.ctx.uniform2fv(this.uniforms[location], values);
            },
            Uniform3fv: (location, values_ptr, values_len)             => { 
				var values = this.mem.loadF32Array(values_ptr, values_len*3);
				this.ctx.uniform3fv(this.uniforms[location], values);
            },
            Uniform4fv: (location, values_ptr, values_len)             => { 
				var values = this.mem.loadF32Array(values_ptr, values_len*4);
				this.ctx.uniform4fv(this.uniforms[location], values);
            },
...
let is_shit_browser = false;
let enable_sound = false;
var screen_scale = 1;

async function _fetch_file(path, index) {
	...
	fetched_files[index] = new Uint8Array(buffer);
}
...
function odinSetupDefaultImports(wasmMemoryInterface, consoleElement) {
	...
	window.odin.WebGLInterface = new WebGLInterface(wasmMemoryInterface);;
	let webglContext = window.odin.WebGLInterface;
	let start_time = Date.now();
	let socket;
	// Must match same named constants in network_js.odin
	const STATUS_CONNECTED    = 1;
	const STATUS_DISCONNECTED = 2;
	const STATUS_CONNECTING   = 3;
	return {
		"js_font": {
			js_generate_font_texture: (texture, texture_dim, font_size, chars_ptr, chars_len, glyphs_ptr, one_glyph_data_size, font_descent_ptr, font_ascent_ptr) => {
				let fc = document.createElement("canvas");
				fc.width = texture_dim;
				fc.height = texture_dim;
				...
				var px = 0;
				var py = 0;
				let one_glyph_float_count = one_glyph_data_size / 4;
				let chars = wasmMemoryInterface.loadString(chars_ptr, chars_len);
				let glyphs_data = wasmMemoryInterface.loadF32Array(glyphs_ptr, chars_len * one_glyph_float_count);
				let ascent = 0;
				let descent = 0;
				for(var ci = 0; ci < chars.length; ci++) {
					...
					let px1 = px+tm.width;
					let fontHeight = tm.fontBoundingBoxAscent+tm.fontBoundingBoxDescent;
					...
					ctx.fillText(text, px, py + tm.fontBoundingBoxAscent);
					let glyph_data_offset = ci * one_glyph_float_count

					glyphs_data[glyph_data_offset+0] = px / fc.width;
					glyphs_data[glyph_data_offset+1] = py / fc.width;
					glyphs_data[glyph_data_offset+2] = (px1) / fc.width;
					glyphs_data[glyph_data_offset+3] = (py+fontHeight) / fc.width;
					glyphs_data[glyph_data_offset+4] = 0;
					glyphs_data[glyph_data_offset+5] = -fontHeight;

					glyphs_data[glyph_data_offset+6] = tm.width;
					glyphs_data[glyph_data_offset+7] = fontHeight;
					glyphs_data[glyph_data_offset+8] = tm.width;
					
					
					px += Math.ceil(tm.width)+2;
				}
				wasmMemoryInterface.storeF32(font_descent_ptr, -descent);
				wasmMemoryInterface.storeF32(font_ascent_ptr, ascent);

				let gl = webglContext.ctx;
				//gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
				gl.bindTexture(gl.TEXTURE_2D, webglContext.textures[texture]);
		...
		/*"gui": {
			js_draw_text: (ptr, len, font_size, x, y, align_x, align_y, r,g,b,a) => {
				//console.log("draw text", x, y, r, g, b, a);
				// TODO: Should the canvas be scaled instead?
				font_size /= screen_scale;
				x /= screen_scale;
				y /= screen_scale;
				const ctx = window.gui_ctx;
				ctx.fillStyle = `rgba(${r*255}, ${g*255}, ${b*255}, ${a})`;
				ctx.font = `${font_size}px Bloody`;
				const x_aligns = [
					"left",
					"center",
					"right"
				...
				ctx.textAlign = x_aligns[align_x];
				ctx.textBaseline = y_aligns[align_y];
				//ctx.rect(20, 20, 400, 200);
				//ctx.clip()
				ctx.fillText(wasmMemoryInterface.loadString(ptr, len), x, y);
			},
			js_push_scissor: (x, y, w, h) => {
				const ctx = window.gui_ctx;
				ctx.save();
				// TODO: Should the canvas be scaled instead?
				ctx.rect(x/screen_scale, y/screen_scale, w/screen_scale, h/screen_scale);
				ctx.clip();
			},
			js_pop_scissor: () => {
				window.gui_ctx.restore();
			}

		},*/
		"network": {
			connect_to_socket: (host_ptr, host_len, recv_buffer_ptr, recv_buffer_size) => {
				let host = wasmMemoryInterface.loadString(host_ptr, host_len);
				console.log("Connecting to: " + host);
				wasmMemoryInterface.exports.on_server_status_changed(STATUS_CONNECTING);
				...
				socket.onmessage = (event) => {
					if(typeof event.data == 'string') {
				    	...
				    	let socket_recv_buffer = wasmMemoryInterface.loadBytes(recv_buffer_ptr, recv_buffer_size);
				    	for(var ii = 0; ii < data.length; ii++) {
				    		socket_recv_buffer[recv_end] = data[ii];
				    		recv_end += 1;
				    	}
				    	wasmMemoryInterface.exports.recv(recv_start, recv_end);
				    	recv_start = recv_end;

				    	if(recv_start > recv_buffer_size/2) {
				    		recv_start = 0;
				    		recv_end = 0;
				    	}
				    }
				...
				socket.onopen = (event) => {
					wasmMemoryInterface.exports.on_server_status_changed(STATUS_CONNECTED);
					console.log("Open");					
				}
				socket.onclose = () => {
					wasmMemoryInterface.exports.on_server_status_changed(STATUS_DISCONNECTED);
			...
			send: (data_ptr, data_len) => {
				if(socket != undefined) {
					socket.send(wasmMemoryInterface.loadBytes(data_ptr, data_len));
				}
			}
		},
		"audio": {
			js_play_sound: (ptr, len, volume) => {
				if(!enable_sound) return;
				let name = wasmMemoryInterface.loadString(ptr, len);
				...
				if(audio == undefined) {
					...
					audio.load();
					sounds[name] = audio;
					//console.log("Init sound", audio);
				}
				//audio.playbackRate = 0.7;
				if(audio.error == undefined || audio.error == null) {
					//console.log(audio);
					audio.volume = volume;
					audio.currentTime = 0;
					audio.play();
			...
			platform_set_sound_volume: (volume) => {				
				sound_volume = volume;
				document.cookie = "sound_volume=" + volume + ";" + "SameSite=Strict;";

				if(music != undefined) {
					music.volume = sound_volume*music_volume;
				}
			},
			platform_set_music_volume: (volume) => {
				music_volume = volume;
				document.cookie = "music_volume=" + volume + ";" + "SameSite=Strict;";
				if(music != undefined) {
					music.volume = music_volume*0.5;
					if (music_volume == 0) {
						music.pause();
					}
				}
		...
		"env": {
			log: (ptr, len) => { console.log(wasmMemoryInterface.loadString(ptr, len)); },
			log_int: (i) => { console.log(i); },
			us_now: () => { return (Date.now()-start_time)*1e3; },
			ms_now: () => { return (Date.now()-start_time); },

			fetch_file: (path_pointer, path_length) => {
				let path = wasmMemoryInterface.loadString(path_pointer, path_length);
				let index = fetched_files.length;
				fetched_files.push(null);
				
				_fetch_file(path, index);
				return index;
			},
			is_file_fetching_done: (index) => { return fetched_files[index] != null; },
			get_fetched_file_length: (index) => { return fetched_files[index].length; }, 
			get_fetched_file_data: (index, buffer) => {
				const ff = fetched_files[index];
				for(let ii = 0; ii < ff.length; ii++) {
					wasmMemoryInterface.storeU8(buffer+ii, ff[ii]);
				}
			},
			js_get_name: (dest_ptr, max_size) => {
				if(name == null) {
					return 0;
				}
				const bytes = wasmMemoryInterface.loadBytes(dest_ptr, max_size);
				let encodeResult = new TextEncoder().encodeInto(name, bytes);
				return encodeResult.written;
				/*let size = new TextEncoder().encode(name).length;
				if(size <= max_size) {
					wasmMemoryInterface.storeString(dest_ptr, name);
					return size;
				}
				else {
					wasmMemoryInterface.storeString(dest_ptr, name.substring(0, max_size));
					return max_size;
				}*/
			},
			js_open_name_input: (dest_ptr, max_size) => {
		...
		"odin_dom": {
			init_event_raw: (ep) => {
				...
				/*if(event_temp_data.name == "touchstart") {
					console.log("Touchstart", e);
				}
				if(event_temp_data.name == "touchend") {
					console.log("Touchend", e);
				}
				if(event_temp_data.name == "touchmove") {
				...
				if (window.TouchEvent && e instanceof TouchEvent) {
					if(e.touches.length > 0) {
						...
						wmi.storeI64(off(8), t.clientX*screen_scale);
						wmi.storeI64(off(8), t.clientY*screen_scale);
					}
					else {
						wmi.storeI64(off(8), 0);
						wmi.storeI64(off(8), 0);
						wmi.storeI64(off(8), 0);
						wmi.storeI64(off(8), 0);
					}
					wmi.storeI64(off(4), e.which);
					...
					wmi.storeI64(off(8), e.pageX*screen_scale);
					wmi.storeI64(off(8), e.pageY*screen_scale);
...
async function runWasm(wasmPath, consoleElement, extraForeignImports) {
	...
	/*const font = new FontFace("Bloody", 'url(res/bloody/Bloody-z0V3.ttf)', {
   	 	style: "normal",
    	weight: "normal",
    //stretch: "condensed",
  	});
  	// TODO: make sure font is loaded when generating glyphs
  	font.load();
	...
	/* Using test() method to search regexp in navigator.userAgent 
	it returns boolean value*/
	is_mobile_device = mobile_regexp.test(navigator.userAgent); 
	let safari_regexp = /safari|Safari/i;
	let apple_regexp = /apple|Apple/i;
	is_shit_browser = safari_regexp.test(navigator.userAgent) && apple_regexp.test(navigator.vendor);
	enable_sound = !is_shit_browser;
	...
	/*if(name == null) {
		name = prompt("What's you name?", "");
		document.cookie = "name=" + name + ";" + "SameSite=Strict;";
	}*/
	//console.log("Name:", name);

	//exports.on_loaded_settings(sound_volume, music_volume, exports.default_context_ptr());
	exports._start(is_mobile_device, enable_sound);

	if(enable_sound) {
		music = new Audio("sounds/fast_bass.mp3");
		music.addEventListener('ended', function() {
		    music.play();
		}, false);
		music.volume = music_volume*3.0;
	}
	//music.play();
	...
	if (exports.step) {
		...
		const step = (currTimeStamp) => {
			...
			let glctx = window.odin.WebGLInterface.ctx;


			if(glctx != undefined) {
				let scale = screen_scale;
				glctx.canvas.width = Math.floor(window.innerWidth*scale);
				glctx.canvas.height = Math.floor(window.innerHeight*scale);
				glctx.canvas.style.width = window.innerWidth + "px";
				glctx.canvas.style.height = window.innerHeight + "px";
			}
...
window.onclick = () => {
	//console.log("FOCUSED");
	if (music != undefined && music.volume > 0) {
		music.play();
